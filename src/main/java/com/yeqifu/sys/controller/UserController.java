...
public class UserController {
    ...
    public DataGridView loadAllUser(UserVo userVo){
        ...
        for (User user : list) {
            ...
            if (gradeId!=null){
                ...
                //设置user的部门名称
                user.setGradename(one.getName());
            }
        }
        ArrayList<User> users = new ArrayList<>();
        if(StringUtils.isNotBlank(userVo.getGradename())){
            users.clear();
            for (User user : list) {
                if(user.getGradename()!=null&&user.getGradename().indexOf(userVo.getGradename())>=0){
                    users.add(user);
                }
            }
            return new DataGridView(users.stream().count(),users);
        }
     ...
     * @return
     */
    @RequestMapping("loadUsersBygradeId")
    public DataGridView loadUsersByGradeIp(Integer gradeId){
        QueryWrapper<User> queryWrapper = new QueryWrapper<User>();
    ...
    public ResultObj addUser(UserVo userVo){
        try {
            ...
            gradeQueryWrapper.eq(StringUtils.isNotBlank(userVo.getGradename()),"name",userVo.getGradename());
            userVo.setGradeid(gradeService.getOne(gradeQueryWrapper).getId());
            userService.save(userVo);
            //给用户添加默认添加上学生角色
            Integer userId = userService.getIdByLoginName(userVo.getLoginname());
            roleService.insertUserRole(userId,Constast.STUDENT_ROLE);
            return ResultObj.ADD_SUCCESS;
        } catch (Exception e) {
            e.printStackTrace();
            return ResultObj.ADD_ERROR_LOGINNAME;
        }
    }
    ...
    public ResultObj updateUser(UserVo userVo){
        ...
        QueryWrapper<Grade> gradeQueryWrapper = new QueryWrapper<>();
        gradeQueryWrapper.eq(StringUtils.isNotBlank(userVo.getGradename()),"name",userVo.getGradename());
        Integer id = gradeService.getOne(gradeQueryWrapper).getId();
        userVo.setGradeid(id);
        try {
